...
const Code = defineAsyncComponent(() => import("@/components/common/Code/index.vue"));
...
async function onConversation() {
  ...
  try {
    ...
    const fetchChatAPIOnce = async () => {
      const responseText = await fetchChatAPIProcess<Chat.ConversationResponse>({
        prompt: dataSources.value.map((item) => item.text).join("\n"),
        options,
        signal: controller.signal,
      });
      if (responseText === "您的code次数已经用完，请重新获取！") {
        showCode.value = true;
      }
      const data = {
        conversationId: "",
...
<template>
  <div class="flex flex-col w-full h-full">
    ...
    <Code v-if="showCode" v-model:visible="showCode" />
  </div>
</template>
